...
export const GET = async (request: Request): Promise<Response> => {
  ...
  const stage = await Stage.findById(stageId)
    .populate({
      path: 'vacancy',
      select: 'title place skills roof_type location salary homePrice home_descr work_descr grafik drivePermis langue workHours documents slug imageFB homeImageFB manager profession city country status',
      populate: {
        path: 'manager',  
